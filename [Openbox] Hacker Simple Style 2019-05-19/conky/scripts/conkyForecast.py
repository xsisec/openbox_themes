		...
		#try:
			...
			if startday == None: # current conditions
				...
				if datatype == "DW":
					...
					if self.locale == "es":
						output = WeatherText.conditions_text_es[self.current_conditions[1].condition_code]
					elif self.locale == "fr":
						output = WeatherText.conditions_text_fr[self.current_conditions[1].condition_code]
					else:
						output = WeatherText.conditions_text[self.current_conditions[1].condition_code]
				elif datatype == "CT":
					output = self.current_conditions[1].condition_text
				elif datatype == "PC":
					string = self.current_conditions[1].precip
					if self.isNumeric(string) == True:
						string = string + u"%"
					output = string
				elif datatype == "HM":
					string = self.current_conditions[1].humidity
					if self.isNumeric(string) == True:
						string = string + u"%"
					output = string
				elif datatype == "WD":
					string = self.current_conditions[1].wind_dir
					if self.isNumeric(string) == True:
